...
- **Bootstrap** новых кластеров. Сначала единственный seed, затем добавление в режиме learner.
...
- **Пулы хранения**: `spec.storage.pools` разносит членов кластера по нескольким `StorageClass` — по одному члену на пул, — чтобы массив хранения был доменом отказа, а не единой точкой отказа. Размещение идёт по правилу «наименее занятый пул» (члены именуются через `GenerateName`, так что порядкового номера, за который можно зацепиться, нет), и это делает seed bootstrap детерминированным и возвращает замену на тот массив, который освободился; `pools[].disabled` ставит кордон на отказавший массив, и замена уходит на здоровый. См. [docs/concepts.md](docs/concepts.md#пулы-хранения-массив-как-домен-отказа).
- **Планирование и дополнительные метаданные**: `spec.affinity` и `spec.topologySpreadConstraints` передаются каждому поду члена как есть (anti-affinity по умолчанию не проставляется — задайте её для продакшена); `spec.priorityClassName` даёт членам класс приоритета, чтобы etcd дочернего кластера не выселялся с ноды под давлением наравне с рабочей нагрузкой; `spec.additionalMetadata` сливает пользовательские метки и аннотации на каждый объект, создаваемый оператором (поды членов, PVC с данными, Service, PDB, объекты `EtcdMember`), причём ключи, принадлежащие оператору, побеждают при столкновении. Все четыре применяются при создании объекта и фиксируются наравне с остальной спецификацией. См. [docs/concepts.md](docs/concepts.md#планирование-подов-и-дополнительные-метаданные).
...
- **[Концепции](docs/concepts.md)** — обоснование устройства: схема фиксации намерения, bootstrap через единственный seed, именование через GenerateName, механика масштабирования в ноль, справочник по условиям.
...
- **[Миграция](docs/migration.md)** — переход на этот оператор со старого оператора aenix присвоением на месте, плюс пример поднятия кластера из уже имеющегося бэкапа в S3 через `bootstrap.restore`; отслеживает изменения поведения, требующие явного шага миграции, — сейчас это требование собственного Secret с учётными данными root при включении аутентификации.
...
- **Bootstrap** — создание единственного seed, идемпотентное восстановление, имена, назначаемые через `GenerateName`.
- **Схема фиксации намерения** — `status.observed` и `progressDeadline` фиксируют цель, к которой идёт оператор; истёкший срок bootstrap терминален.
...
- **Discovery** — seed находится по `spec.bootstrap=true`; эндпоинты клиента etcd отфильтрованы до голосующих (`MemberReady=True`), чтобы `MemberList` не попал на learner.
