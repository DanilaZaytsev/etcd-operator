...
| `kubeRbacProxy.image.repository` | `ghcr.io/kube-rbac-proxy/kube-rbac-proxy` | Репозиторий образа sidecar'а метрик. В закрытом контуре зеркалируйте и его: под оператора не станет Ready, пока не загрузится этот контейнер, даже если сам менеджер уже работает. |
...
Оператор сначала бутстрапит единственный seed, фиксирует `clusterID`, затем добавляет остальных членов по одному как learner (с повышением). Кластер из трёх членов на здоровой инсталляции обычно достигает `READY=3` заметно быстрее минуты.
...
Установка в закрытом контуре тянет **три** образа, и забыть про третий легко, потому что отказ выглядит не как отказ загрузки:

1. образ оператора (`image.repository`) — он же выполняет Job снапшотов и init-контейнеры восстановления;
2. образ etcd (`etcdImage.repository`) — для подов членов и для `etcdutl` при восстановлении;
3. образ `kube-rbac-proxy` (`kubeRbacProxy.image.repository`) — sidecar, закрывающий `/metrics`.

Третий по умолчанию берётся с `ghcr.io`. Если реестр недоступен, менеджер в поде поднимается нормально, а под целиком остаётся `1/2` и никогда не становится Ready — `helm upgrade` при этом висит до таймаута с `Pending termination`. Диагностируется по `kubectl describe pod`: `ImagePullBackOff` будет на контейнере `kube-rbac-proxy`, не на `manager`. Либо зеркалируйте образ, либо выключите прокси через `kubeRbacProxy.enabled=false` — тогда менеджер отдаёт метрики открытым текстом на `:8080`.
