...
(`etcd-migrate`), **пример поднятия кластера из существующего бэкапа в S3**
(когда etcd работала без оператора и данные нужно сохранить),
**переименование `EtcdBackup` → `EtcdSnapshot`** (изменение имени до GA),
переход **`spec.options` из карты в типизированные поля** и то единственное
изменение, у которого есть жёсткое требование к миграции: **учётные данные
аутентификации etcd**.
...
4. **Создаёт новые CR с заранее заполненным статусом**: `EtcdCluster` получает
   `status.clusterID`, `clusterToken` и `observed` (чтобы ветка bootstrap в
...
## Пример: поднять кластер из существующего бэкапа в S3

Сценарий: etcd жила без оператора — например, тремя отдельными `Deployment` с
общим Service перед ними, — а её снапшоты складывались в S3. Новый
`EtcdCluster` засевается одним из этих снапшотов и дальше живёт под оператором.

Восстановление возможно **только при первом bootstrap**: `spec.bootstrap`
неизменяем после создания, поэтому засеять уже работающий кластер нельзя.
Порядок — проверить снапшот, создать кластер с `bootstrap.restore`, переключить
потребителей.

### Ключ адресует один объект, а не префикс

У `s3.key` смысл зависит от того, на какой стороне жизненного цикла снапшота он
стоит, и на этом чаще всего спотыкаются:

- как **назначение** (`EtcdSnapshot.spec.destination`,
  `EtcdSnapshotPolicy.spec.destination`) `key` — это префикс, к которому агент
  сам дописывает `<имя-снапшота>.db`;
- как **источник восстановления** (`spec.bootstrap.restore.source`) `key` — это
  полный ключ объекта целиком, вместе со всеми папками, но **без** схемы
  `s3://<bucket>/` и **без** ведущего `/`.

Поэтому раскладка вида «папка родительского кластера → папка арендатора →
файлы» переносится в `key` дословно: `<parent>/<tenant>/<файл>.db`. Подстановки
«последнего» снапшота нет — имя файла называется явно. Пустой локатор отвергает
и CEL на apiserver, и сам агент восстановления, чтобы bootstrap не залипал на
непонятной ошибке S3 внутри init-контейнера. То же и у источника на PVC:
`pvc.subPath` — полный путь к файлу `.db` внутри тома, а не каталог.

### Шаг 1. Проверьте снапшот и посчитайте контрольную сумму

Агент восстановления умеет ровно один формат — сырой `.db`, снятый
`etcdctl snapshot save` (или агентом снапшота этого оператора). Ни `gzip`, ни
`tar` он не разворачивает, поэтому архив нужно распаковать и положить рядом.

```sh
aws --endpoint-url https://s3.example.com s3 ls s3://<bucket>/<parent>/<tenant>/
aws --endpoint-url https://s3.example.com s3 cp \
  s3://<bucket>/<parent>/<tenant>/<файл>.db /tmp/snapshot.db

etcdutl snapshot status /tmp/snapshot.db -w table   # revision и totalKey — значит, формат тот
sha256sum /tmp/snapshot.db                          # значение пойдёт в bootstrap.restore.checksum
```

Если `etcdutl snapshot status` ругается, файл сжат или обрезан — восстановление
из него не поднимется, а упадёт внутри init-контейнера `restore`.

### Шаг 2. Secret с доступом к S3

Он читается **в неймспейсе кластера**, и ключи в нём именно эти — агент получает
их как переменные окружения, а не по произвольным именам:
...
### Шаг 3. Остановите пишущих и снимите финальный снапшот

Регулярный бэкап — это состояние на момент своего снятия; всё, что записано
после него, потеряется. Поэтому последний снапшот снимается уже после остановки
пишущих, и восстанавливаться нужно именно из него.

```sh
# 1. Остановить тех, кто пишет в старую etcd (пример для control plane в Kamaji)
kubectl patch tenantcontrolplane <tcp> -n <ns> --type=merge \
  -p '{"spec":{"controlPlane":{"deployment":{"replicas":0}}}}'

# 2. Снять снапшот с любого живого члена (добавьте --cacert/--cert/--key, если у него TLS)
kubectl exec -n <ns> deploy/<старый-etcd-1> -- \
  etcdctl --endpoints=127.0.0.1:2379 snapshot save /tmp/final.db
kubectl cp <ns>/<под>:/tmp/final.db /tmp/final.db

# 3. Положить рядом с остальными бэкапами и посчитать контрольную сумму
aws --endpoint-url https://s3.example.com s3 cp /tmp/final.db \
  s3://<bucket>/<parent>/<tenant>/etcd-final-2026-09-16.db
sha256sum /tmp/final.db
```

Старую etcd на этом шаге **не удаляйте** — пока новый кластер не отдал
`Available=True`, она остаётся единственным быстрым откатом.

### Шаг 4. Создайте `EtcdCluster`
...
spec:
  replicas: 3
  version: "3.6.11"              # НЕ НИЖЕ версии etcd, из которой снят снапшот
  storage:
    size: 10Gi                   # примерно 2 размера снапшота на время восстановления
    storageClassName: <класс>    # StorageClass для PVC каждого члена
  tls:                           # новый кластер поднимается с mTLS
    peer:
      certManager:
        issuerRef: { name: etcd-ca-issuer }
    client:
      certManager:
        serverIssuerRef: { name: etcd-ca-issuer }
        operatorClientIssuerRef: { name: etcd-ca-issuer }
  bootstrap:
    restore:
      checksum: "sha256:<из шага 3>"
      source:
        s3:
          endpoint: https://s3.example.com
          bucket: <bucket>
          key: <parent>/<tenant>/etcd-final-2026-09-16.db   # ТОЧНЫЙ ключ объекта
          region: us-east-1
          forcePathStyle: true                              # MinIO/Ceph — обычно да
          credentialsSecretRef:
            name: s3-creds
```

**StorageClass задаётся в `spec.storage.storageClassName`** — по нему создаётся
PVC каждого члена. Семантика та же, что у
`PersistentVolumeClaim.spec.storageClassName`: поле опущено — берётся класс по
умолчанию неймспейса, пустая строка — динамическое выделение отключено (PV
должен быть подготовлен заранее), любое другое значение называет конкретный
класс. Поле **неизменяемо после создания**: `storageClassName` у самого PVC
изменить нельзя, а перекатку PVC оператор не выполняет, — поэтому класс
выбирается сейчас, а не потом.

Когда членов нужно развести по разным массивам, вместо одного класса задаются
пулы (одно исключает другое):
...
Seed детерминированно попадает в `pools[0]`, а восстанавливается именно он —
поэтому первым ставьте тот массив, на котором точно есть место под снапшот и
пересобранный data-dir. Разбор пулов — в
[концепциях](concepts.md#пулы-хранения-массив-как-домен-отказа).

Блок `spec.tls` здесь в режиме cert-manager; вариант с собственными Secret и
команды их создания — в [кластерах с TLS](operations.md#кластеры-с-tls).

### Шаг 5. Дождитесь восстановления и переключите потребителей

Восстанавливается **только seed**: его data-dir пересобирается `etcdutl` из
целевого образа etcd, после чего остальные члены присоединяются к живому
кластеру обычным порядком.

```sh
kubectl get etcdcluster etcd -n <ns> -w
kubectl logs -n <ns> <под-seed> -c restore        # пересборка data-dir
kubectl logs -n <ns> <под-seed> -c install-tools  # если "restore" не стартовал

# Дождавшись Available=True, сверьте ревизию — она должна быть близка к ревизии снапшота
kubectl etcd endpoint status --cluster etcd -n <ns>
```

Клиентский Service оператора называется `<cluster>-client`, поэтому строка
подключения меняется — у `DataStore` в Kamaji это одно поле:

```yaml
spec:
  endpoints:
    - etcd-client.<ns>.svc:2379
  tlsConfig: { ... }   # CA и клиентский сертификат нового кластера
```

Верните пишущих в строй, убедитесь, что они поднялись на восстановленных
данных, и только после этого удаляйте старые `Deployment`, их Service и PVC.
Заведите `EtcdSnapshotPolicy` на тот же префикс `<parent>/<tenant>/`, чтобы
бэкапы продолжились уже средствами оператора — порядок в [регулярных
бэкапах](operations.md#регулярные-бэкапы).

### Что сорвёт восстановление

- **Ключ с `s3://<bucket>/` или с ведущим `/`.** Агент запросит несуществующий
  объект; ключ пишется от корня бакета.
- **Архив вместо `.db`.** Распаковка на пути восстановления не предусмотрена.
- **`spec.version` ниже версии источника.** `etcdutl` целевого образа всегда в
  связке с той etcd, что стартует на результате, но версия происхождения
  снапшота нигде не записана: более старый `etcdutl` над базой более нового
  минора не проверялся и не поддерживается.
- **Включённая аутентификация в старой etcd.** Снапшот несёт bcrypt-хэш пароля
  root, поэтому новому кластеру нужен `spec.auth` с **тем же** паролем (а с ним
  транзитивно и `spec.tls.client`). Иначе оператор получает
  `Available=False`/`Degraded=True` с причиной `AuthRequiredNotConfigured`, а
  так как `spec.auth` неизменяем после создания, лечится это только
  пересозданием. Подробности — в [учётных данных
  root](#аутентификация-учётные-данные-root-свои-и-обязательны).
- **Том меньше примерно двух размеров снапшота.** Снапшот из S3 скачивается на
  сам том данных, а не в эфемерный `/tmp` контейнера, поэтому на время
  пересборки занято около двойного объёма. Агент проверяет это через
  `HeadObject` до начала загрузки и падает заранее с внятным сообщением.

После восстановления etcd назначает **новый cluster ID**: это свежий кластер,
засеянный старыми данными, а не продолжение прежнего. Полный разбор пути
восстановления — в [порядке восстановления из
снапшота](operations.md#восстановление-кластера-из-снапшота).
