...
# Взгляд на уровне членов: флаг bootstrap, спящее состояние, готовность.
...
- «no data has been written (cluster never bootstrapped)» → кластер с самого начала создавали с `replicas=0`; увеличение запустит свежий bootstrap.
...
Discovery при bootstrap не смогло подключиться до seed. Сообщение приходит через `stableErrorMessage(err)`, который убирает изменчивые от вызова к вызову части (метки времени, номера портов), поэтому одна и та же первопричина читается одинаково при повторах.

```sh
kubectl get etcdcluster.etcd-operator.cozystack.io <name> -n <ns> \
  -o jsonpath='{.status.conditions[?(@.type=="Available")].message}{"\n"}'
kubectl get pod -l etcd-operator.cozystack.io/cluster=<cluster> -n <ns>
kubectl logs -n <ns> <под-seed>
```

Если под seed `Running 1/1`, а контроллер по-прежнему сообщает `ClusterUnreachable`, подозревайте проблему со Service или DNS:
...
kubectl run dns-debug --rm -it --image=busybox -n <ns> -- \
  nslookup <имя-пода-seed>.<cluster>.<ns>.svc.cluster.local
```

### `Available=False/BootstrapFailed`

Deadline истёк раньше, чем был зафиксирован `clusterID`, **и** Pod seed не отвечает. Отвечающий seed сюда не попадает — оператор продлевает окно сам и уходит в `Progressing=True/BootstrapRetrying`, поэтому это состояние означает именно «seed не поднялся».

Начинайте с seed — почти всегда дело в нём, а не в операторе:
...
kubectl get pvc -n <ns> -l etcd-operator.cozystack.io/cluster=<cluster>
```

Обычные причины: PVC не привязывается (нет `StorageClass`, исчерпана квота облака на диски), образ не загружается, спецификация пода отклонена допуском PodSecurity. Почините причину — как только seed станет Ready, оператор подхватит кластер сам, без правок спецификации.

Если причина в самой спецификации (несуществующий `storageClassName`, недостижимый репозиторий образа), исправьте её — правка принимается и на этой фазе:

```sh
kubectl edit etcdcluster.etcd-operator.cozystack.io <name> -n <ns>
```

Удаление и пересоздание кластера остаётся крайней мерой: оно уносит PVC. Если всё же идёте этим путём, дождитесь исчезновения прежних PVC, иначе новый EtcdMember откажется их присваивать (проверка UID в `pvcOwnedBy` не проходит — см. [концепции](concepts.md#модель-api)).

### `Available=False/DeadlineExceeded`

Deadline истёк уже после того, как кластер собрался: сам кластер здоров, застряла только последняя операция. Оператор паркуется и ждёт правки спецификации:
...
Как только под seed создан, `WaitingForSeed` уходит, и проблемы *на стороне пода* (отсутствующий `StorageClass`, ошибка загрузки образа, отказ PodSecurity в допуске `restricted`-совместимой спецификации) всплывают отдельно. С этого момента правильный инструмент — `kubectl describe pod -l etcd-operator.cozystack.io/cluster=<cluster> -n <ns>`.
...
Это немедленно переводит кластер в терминальное состояние ошибки. Дальше — по соответствующей ветке условий выше (до того как кластер собрался — починить seed или править спецификацию; после — править спецификацию).
...
| `etcd_operator_cluster_broken_members{namespace,cluster}` | `status.brokenMembers`: члены, у которых **был** Pod и которые не обслуживают, в отличие от тех, кого ещё не создали. |
| `etcd_operator_cluster_generation{namespace,cluster}` | `metadata.generation` — спецификация, которую попросил пользователь. |
| `etcd_operator_cluster_observed_generation{namespace,cluster}` | `status.observedGeneration` — спецификация, по которой оператор довёл до конца проход reconcile. |
...
`broken_members` отвечает на вопрос, который `members_ready` закрыть не может: «готовы 2 из 3» выглядит одинаково и когда третий член ещё создаётся, и когда у него есть Pod, который не обслуживает. Первое пройдёт само, второе нет.

Метрика намеренно мгновенная, а не накопительная: член, который штатно перезапускается при drain или обновлении образа, проходит через это состояние несколько секунд. Поэтому алерт на неё требует, чтобы состояние держалось 15 минут. Предикат ничего не решает — самолечение при crash-loop живёт в контроллере члена и работает независимо; `broken_members` только сообщает, что происходит.

Две серии про generation экспортируются раздельно, а не одной разницей: так выражение алерта читается и сверяется с `kubectl get etcdcluster -o yaml` напрямую, а кластер, спецификацию которого никогда не меняли, не выглядит застрявшим. Отставание `observed_generation` от `generation` дольше нескольких минут — это «оператор не доработал спецификацию до конца», и ловится оно одним правилом, без перечисления причин. На прогоне это был единственный сигнал, отличавший припаркованный кластер от здорового: поды у него исправны и число готовых членов не отличается.
...
| Метрика | Значение |
|---|---|
| `etcd_operator_defrags{namespace,cluster,phase}` | Сколько объектов EtcdDefrag в каждой фазе для кластера. |
| `etcd_operator_defrag_policy_info{namespace,policy,cluster,schedule,timezone}` | Всегда 1. Расписание, по которому политика реально работает. |
| `etcd_operator_defrag_policy_suspended{namespace,policy}` | 1, пока политика приостановлена. |
| `etcd_operator_defrag_policy_last_schedule_timestamp_seconds{namespace,policy}` | Когда политика в последний раз отработала наступивший тик. |
| `etcd_operator_defrag_policy_last_success_timestamp_seconds{namespace,policy}` | Когда defrag по этой политике в последний раз завершился успешно. |
| `etcd_operator_defrag_policy_active_defrags{namespace,policy}` | Сколько defrag'ов этой политики ещё выполняется. |

Набор намеренно повторяет форму метрик политики бэкапов: вопросы к обеим одинаковые, и одинаковая форма означает, что и алерты к ним пишутся одинаково. Как и там, у политики, которая ни разу не отработала успешно, серии `last_success` нет вовсе — ловить это надо через `unless`, а не сравнением с нулём.

Defrag — единственная операция обслуживания, которую оператор выполняет против живого кластера, и до этого он не отдавал по ней ни одной метрики. База etcd не возвращает освобождённые страницы файловой системе сама, поэтому политика, которая тихо перестала срабатывать, проявляется как рост `db size` месяцы спустя.
...
Восстановление возможно **только при первом bootstrap**: *новый* кластер инициализирует data-dir своего seed из снапшота вместо пустого старта. Восстановиться в существующий, уже забутстрапленный кластер нельзя (`spec.bootstrap` неизменяем после создания) — удаляйте и создавайте заново.
...
# Под seed запускает два init-контейнера до старта etcd, именно в этом порядке:
...
kubectl logs -n <ns> <под-seed> -c restore
# Если "restore" так и не стартовал, не удалась раскладка — смотрите туда:
kubectl logs -n <ns> <под-seed> -c install-tools
```
...
- Агент восстановления пересобирает data-dir через `etcdutl`, используя идентичность seed (имя, initial-cluster, токен, peer URL), после чего etcd стартует на нём. Члены, добавляемые масштабированием позже, присоединяются к живому кластеру обычным порядком — восстанавливается только seed.
...
**5. Создайте заново с тем же именем и источником восстановления.** Имя то же, чтобы клиентский Service — а значит, и строка подключения, которую держит control plane дочернего кластера, — не изменился. Перенесите `spec.auth`, `spec.tls` и, если кластер их использовал, те же `spec.storage.pools` в том же порядке: seed детерминированно попадает в `pools[0]`, и именно это делает процедуру воспроизводимой.
...
**6. Понаблюдайте за восстановлением seed, затем за сборкой кластера.**

```sh
kubectl get etcdcluster <cluster> -n <ns> -w
kubectl logs -n <ns> <под-seed> -c restore        # пересборка
kubectl logs -n <ns> <под-seed> -c install-tools  # если "restore" не стартовал
```
...
- **Дальше восстановление идемпотентно.** Как только data-dir инициализирован, init-контейнер ничего не делает, поэтому перезапуски пода seed не перезагружают снапшот и не затирают живые данные.
...
|---|---|
| `bootstrapping single-node cluster` | Bootstrap на первом reconcile создаёт seed. |
| `waiting for bootstrap member to form cluster` | Seed создан, ждём его Pod и discovery через etcd. |
| `cluster declared paused from the start; not bootstrapping` | `spec.replicas=0` на кластере, который никогда не собрался. |
...
- **Seed не исключение**: то, что он сформировал кластер, — факт происхождения, а не постоянная индульгенция. Защищено только *окно* bootstrap, и делает это один лишь гейт по кворуму: до фиксации `clusterID` ничто ещё не считалось готовым, поэтому `ReadyMembers` равен 0 и не проходит ни один член. Та же арифметика постоянно защищает единственного члена кластера с одной репликой.
...
Отдельно: граница привязана к *предполагаемому* размеру кластера, а не только к живому числу голосующих. Пока кластер ниже цели — bootstrap, увеличение или ротация нод, убравшая членов быстрее, чем оператор успел восполнить, — граница остаётся на кворуме цели даже при падении числа здоровых голосующих, поэтому бюджет ужимается с каждым недостающим и достигает нуля, как только здоровых голосующих становится `⌊цель/2⌋ + 1`. На цели в 3 члена это любая нехватка вообще; на больших целях нужна нехватка больше одного (цель 5 при 4 здоровых голосующих всё ещё разрешает 1 вытеснение, цель 7 при 6 разрешает 2). Drain, застрявший здесь, — это работающий бюджет; дождитесь, пока оператор восстановит недостающих голосующих.
...
### Найти seed работающего кластера
...
Замечание: после bootstrap у seed нет особой эксплуатационной роли — см. [концепции](concepts.md#именование-членов). Это для исторической справки, а не для маршрутизации.
