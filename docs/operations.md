...
| `etcd_operator_cluster_broken_members{namespace,cluster}` | `status.brokenMembers`. **Сегодня это всегда 0 на кластерах с PVC**: предикат за этим полем (`isBroken`) для них — заглушка. Метрика есть, потому что поле есть в API статуса; алерт по ней писать нельзя, для «членов не хватает» есть `members_ready` против `members_desired`. |
...
Про `broken_members` надо сказать прямо: предикат за этим полем сегодня заглушка для кластеров на PVC, и метрика для них тождественно равна нулю. Проверено на прогоне — при полной потере данных члена, который затем был заменён, она не поднялась ни разу. Метрика экспортируется, потому что поле входит в API статуса и станет осмысленным, если предикат разовьётся; строить на ней алерт нельзя. Алерт, который структурно не может сработать, хуже отсутствующего: он выглядит как покрытие.
